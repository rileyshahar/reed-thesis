...
correctly implemented in any category over any object which satisfies this.
...
\subsection{Modifications to the Framework}

There are several tools, some suggested by~\cite{broadbent-karvonen-2022} and
some which we believe are novel, which may be used to modify the framework.

\subsubsection{Unbounded-Use Resources}
In the title of
...
there are cases where we are fine with having unbounded-use resources. As
suggested by~\cite{broadbent-karvonen-2022}, we can straightforwardly modify the
...
logic, and to have fine-grained control over resource usage. In this case,
unfortunately, there are two separate categories $\ncomb{\cat{C}}$ and
$\ncombb{\cat{C}}$, and it is not immediately clear how to correctly combine
them to allow the form of restricted reasoning common in programming languages
based on linear type systems. We briefly discuss an obstacle, but think this is
...
settings, $!$ can be modeled by a lax monoidal comonad $\denote{!}$ satisfying
certain properties.
% \footnote{
%   Specifically, $\denote{!}$ should be \emph{subunital}, in that there
%   is a natural transformation $\denote{!}x\to I$, and \emph{horizontally idempotent},
%   in that there is a natural isomorphism $\denote{!}x\otimes\denote{!}x\cong
%   \denote{!}x,$ subject to a coherence axiom given in \cite[Equation 72]{mellies-2009}.
% }
The modern perspective of~\cite{benton-1995} is to focus on resolutions of this comonad, i.e.
monoidal adjunctions \[
  \begin{tikzcd}
    \cat{I}\ar[rr, bend left, "F"] & \bot & \cat{L}\ar[ll, bend left, "G"]
  \end{tikzcd}
\]such that $FG = \denote{!}$, and in particular on resolutions such that
$\cat{I}$ is a category with finite products, hence a model of conjunctive
intuitionistic logic\footnote{
  Normally we consider such models with significantly more structure than just
  the multiplcatives, but the situation is the same even in this case.
}. In this way, $F$ is an
embedding of intuitionistic terms as linear terms, while $G$ forgets the linearity of
a term. A standard example of this structure is the free-forgetful adjunction
between \scat{Set} and $\scat{Vect}_\kk$~\cite{valiron-zdancewic-2014}.

All this machinery suggests that, to give a system which allows simultaneous
reasoning about both single- and unbounded-use resources, we should look for
...
n-comb. Furthermore, the category $\ncombb{\cat{C}}$ has is cartesian monoidal,
...
However, the functor $G$ seems unlikely to be a right adjoint\footnote{
  I would like an explicit construction of a limit which $G$ does not preserve;
  the issue is that it seems hard for $\ncomb{\cat{C}}$ to have very many limits
  in the first place. This is an open question.
...
we need a notion of intuionistic resource internal to $\ncomb{\cat{C}}$. A solution
may perhaps be along the lines the $\infty$-combs of~\cite{roman-2020}, which
...
representing intuitionistic resources of type $A\to B$, and which are used to build
n-combs as in $\ncombb{\cat{C}}$. This is a standard strategy in linear logic,
but we spell it out here. Objects in the category
$\ncomba{\cat{C}}$ are finite multisets\footnote{
  We use multisets instead of lists to simplify the monoidal structure; the
  distinction is not important.
} of pairs $(A, B)$ and/or $!(A, B)$ of objects in $\cat{C}$, called
\emph{linear} and \emph{intuitionistic} resources respectively. Letting $i,j,k,$
and $l$ range over finite disjoint index sets $I,J,K,$ and $L$, we want to
describe morphisms
 \[
   \{!(A_i, B_i), (C_j, B_j)\}\to \{!(X_k, Y_k), (Z_l, W_l)\};
\] the domain, for instance, should be read as a multiset of $I$ intuitionistic
resources and $J$ linear resources. To specify such a morphism, we first give a
relation $\alpha: K\sqcup L\to I\sqcup J$, such that each $j\in J$ relates to
exactly one element, and such that each $k\in K$ relates only to elements in
$I$. The point is that each linear resource must be used in exactly one comb,
while we can only build intuitionistic resources out of intuitionistic
resources.

Now the morphism consists of a multiset of $K\sqcup L$ combs. For each $k\in K$, the
comb is constructed just as in $\ncombb{\cat{C}}$, using a function $\sigma:
n\to |\alpha(k)|$ which describes which resources to use in which order. The
...
This category is strict symmetric monoidal with the union of multisets as the
product and the empty set as the identity. Furthermore, there is a
linear-non-linear adjunction \[
  ...
  \end{tikzcd}
\]
where $G$ sends forgets the difference between linear and intuitionistic
resources, and $F$ sends all resources to intuitionistic ones.
