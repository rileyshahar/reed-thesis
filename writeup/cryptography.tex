...
\begin{ex}[pseudorandom generators~{\cite[Sections 3.2-3.3]{pass-shelat-2020}}]\label{ex:pseudorandomness}
  ...
  For simplicity, we now work over $\cX_n = \ZZ_2^n$. Next, let $G:
  \ZZ_2^*\to\ZZ_2^*$ be a sequence of \emph{deterministic} functions. We say $G$
...
\begin{ex}\label{ex:non-zero-knowledge}
  ...
  $P[y = x]\leq 2^{-n}$, which is possible by the pigeonhole principle. Let
  $\cD$ output $1$ on input $\{1^n, x\}$, and $0$ otherwise. Then \[
    \Pr[\cD(\cS(1^n)) = 1] = \Pr[y = x] \leq 2^{-n},
  \] while \[
    \Pr[\cD(\view_\cB(1^n, x)) = 1] = 1.
  \] Since $|1 - 2^{-n}|$ is definitely not negligible, the protocol is not
...
\begin{ex}
  ...
  \begin{itemize}
    \item For the secure-but-not-correct scheme, simply have each machine do
      nothing. The security proof is exactly the same as
      in~\Cref{ex:trivial-protocol}.
    \item For the correct-but-not-secure scheme, have $\cA$ send $m$ to $\cB$ (and
...
\begin{ex}[the bootstrap one-time-pad]\label{ex:botp}
  ...
  $\cA$ first computes $\cG(k)$ to get a key $k'\in\ZZ_2^{p(n)}$. It then
  sends $c = m + k$ to $\cB$. Similarly, $\cB$ computes $k'$ and then $c - k'$,
  which it outputs.

  Since $\cG$ is deterministic, both $\cA$ and $\cB$ get the same value for
  $k'$, and so the protocol is correct. Security can be shown by essentially
  repeating the argument of\label{ex:otp} while carrying a
  negligible error rate due to the pseudorandomness, but an easier route is
  available.

  By definition of pseudorandomness, the distributions $\{\cG(\ZZ_2^n)\}$ and
  $\{\ZZ_2^{p(n)}\}$ are computationally indistinguishable. To obtain $c$ in
  this protocol and in the one-time pad, we then perform the same computation to
  both distributions---merely adding the fixed message $m$. Since we perform the
  same computation to indistinguishable distributions,
  ...
  we obtain the desired result by security of the one-time pad and transitivity of
...
they each output the value $y_i\in Y_i$.
