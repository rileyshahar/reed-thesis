...
\begin{ntn}
	As in \Cref{prot:ideal commitment}, we will use curly letters to denote
...
% 	following hold: \[
% 		\alg{Com}_{S_n}^{R_n}(m) \compind\alg{Com}_{\cS_n}^{\cR_n}(m), \quad\quad
% 		\alg{Rev}_{S_n}^{R_n}(m) \compind\alg{Rev}_{\cS_n}^{\cR_n}(m).
% 	\]
% \end{dfn}
