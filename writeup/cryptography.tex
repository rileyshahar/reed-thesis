...
for any choice of inputs $(x, x_1, \dots, x_n)$, there exists a non-uniform PPT
algorithm $\cS$ such that \[
...
\begin{dfn}[Zero-Knowledge]\label{def:zero knowledge}
  ...
  \emph{zero-knowledge secure for party $i$ relative to $f$} if for any choice
  of inputs $(x, x_1, \dots, x_n)$, there exists a non-uniform PPT algorithm
  $\cS$ such that \[
...
of each of the algorithms in the protocol, but we will not need to be so precise
here.
