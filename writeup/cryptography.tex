...
\begin{questions}
  \item What kinds of protocols are being composed? Our security definitions do not
    ...
    our composition theorem to a specific class of protocols.
  \item What does it mean to compose these protocols? It is not immediately
    clear how to compose arbitary interactive algorithms.
  ...
  \item What kind of security is being preserved? Given a security definition
    ...
    definition for the composite protocol.
  ...
  \item What protocols are we allowed to compose with? We could be allowed to
    ...
    with other protocols we already know are secure.\label{item:compose-with}
  \item How many times we can compose---for instance, must it be constant in the
    security parameter?\label{item:count-compositions}
\end{questions}
...
In regards to~\Cref{item:compose-with}, we need to have some kind of
...
In regards to~\Cref{item:count-compositions}, we should at best expect
...
an bijective map from the parties in $\rho$ to those in $\phi$. Then the
protocol $\pi^{\rho\to\phi}$ replaces the algorithms in $\rho$ with the
corresponding algorithms from $\phi$. This is called the \emph{universal composition
operation.} It takes rather a lot of work to make this substitution operation
precise---for instance, there is a very technical compatibility condition which
essentially asks that the interfaces of $\phi$ and $\rho$ are ``functional'', in
the sense that they only interact with the rest of the protocol via their
inputs and outputs (and the adversary). In~\cite{canetti-2020}, such protocols
are called \emph{subroutine respecting}.

\begin{thm}[the univeral composition theorem]
  Under technical assumptions, if $\rho$ is a subroutine of $\pi$ and $\phi$
  UC-emulates $\rho$, then $\pi^{\rho\to\phi}$ UC-emulates $\pi$.
\end{thm}
