...
\def\multimorphism[#1,#2][#3][#4][#5][#6] {
    ...
    \draw (#3.north west) to ++(-.5,0) node[left] (#3x1) {#4};
    \draw (#3.south west) to ++(-.5,0) node[left] (#3xn) {#5};
    \draw (#3.east) to ++(.5,0) node[right] (#3y) {#6};
    \node[anchor=center,left of=#3,yshift=1mm,xshift=2mm] {$\vdots$};
  }

\def\wire[#1][#2][#3] {
  \draw ($(#1.east)+(.5,0)$) to[out=0,in=180] ($(#2.#3 west)+(-.5,0)$);
}
