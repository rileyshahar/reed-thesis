...
\begin{ex}
  ...
  \begin{itemize}
    ...
    \item Let $L$ be a strongly-typed functional programming language with
      ...
      calculus. Then the category $\cL$ is monoidal with the product as the
      tensor and the unit type as the unit.
  \end{itemize}
\end{ex}
...
Note that there are \emph{four} possible braids we could draw involving $x$ and
...
A sensible choice is to ask for a natural isomorphism $\phi_{x,y}:
...
As usual, it is often easier to understand the algebric structure diagramtically
...
% \subsection{Theories of Resource Conversions}

% Our main use for multicategories will be the following procedure.

% \begin{con}[{\cite[Definition 3.20]{coecke-2016}}]
%   Let $\cat{C}$ be a multicategory. Then there is an associated
%   \emph{monoidal category of linear resource conversions}
%   $\cat{C}^\otimes$ defined as follows:
%   \begin{itemize}
...
%       $\alpha_f: \{1,\dots,n\}\to\{1,\dots,m\}$ and for each $i\in\{1,\dots,m\}$,
%       an enumeration $\{\sigma_i(1),\dots,\sigma_i(k)\}$ of $\alpha^{-1}(i)$ and a morphism \[
%         x_{\sigma_i(1)}, \dot, x_{\sigma_i(k)}\xto{f_i} y_i,
%       \] called the \emph{$y_i$ component};
...
%     \item the tensor product of $x_1\otimes\dots\otimes x_n$ and
%       $y_1\otimes\dots\otimes y_m$ is given by the concatenation
%       $x_1\otimes\dots\otimes x_n\otimes y_1\otimes \dots\otimes y_m$;
%     \item the tensor product of $x_1\otimes\dots\otimes x_n\xto{f} y_1\otimes\dots\otimes y_m$
...
% %   Funcotriality of the tensor is 
% % \end{proof}
% % \section{Resource Theories}
