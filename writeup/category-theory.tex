...
\begin{dfn}
  ...
  \] which commutes with the multiplicative structure of $S^n$, in that \[
...
This last equality requires some explanation. We have a map $f$ and a
composition-compatible list of maps $g_1,\dots,g_n$. If we permute the inputs to
$f$ by $\sigma$, then we need to permute the $g_i$s in the same way. In the most general
case, we could also have further permutations $\tau_i$ to the inputs of each of the
$g_i$s; we need to permute those by $\sigma$ as well. This gives us the left
hand side.

On the right hand side, we first take the composite of $f$ and the $g$s, and
